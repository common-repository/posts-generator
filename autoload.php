...
spl_autoload_register(function( $class_name ){
    ...
    if ( false === strpos( $class_name, ucfirst( ID ) ) ) return;
    ...
    $namespace 	= '';
    $file_name  = '/class-init.php'; // Defaults file name
    ...
    for ( $i = count( $file_parts ) - 1; $i > 0; $i-- ) {
        ...
        if ( count( $file_parts ) - 1 === $i ) {
            ...
            if ( strpos( $name_part , strtolower( ID ) ) !== false ) {

                $file_name = '/class-' . str_replace( array( 'class', $prefix, '_', '-', ( ID ) ), '', strtolower($name_part) ) . '.php';
